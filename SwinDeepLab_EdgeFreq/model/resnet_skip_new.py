...
class StdConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        # 记录该卷积层期望的输入通道数（从权重中获取）
        self.expected_in_channels = self.weight.shape[1]
        # 标记是否为输入层（输入层期望3通道）
        self.is_input_layer = (self.expected_in_channels == 3)

    def forward(self, x):
        if x.dim() == 4:
            B, D1, D2, D3 = x.shape
            # print(f"进入StdConv2d的维度: (B={B}, D1={D1}, D2={D2}, D3={D3})")
            ...
            if self.is_input_layer:
                ...
                # 对中间层：仅确保维度顺序正确（通道数位于索引1）
                # 中间层通道数应与权重期望的一致（如32、64等）
                if D1 != self.expected_in_channels:
                    # 寻找与期望通道数匹配的维度
                    if D2 == self.expected_in_channels:
                        x = x.permute(0, 2, 1, 3)  # (B, H, C, W) → (B, C, H, W)
                    elif D3 == self.expected_in_channels:
                        x = x.permute(0, 3, 1, 2)  # (B, H, W, C) → (B, C, H, W)

            # 验证处理后的维度
            B, C, H, W = x.shape
            # print(f"StdConv2d处理后的维度: (B={B}, C={C}, H={H}, W={W})")
            assert C == self.expected_in_channels, \
                f"通道数不匹配: 期望{self.expected_in_channels}, 实际{C}"
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # 关键修正：在解析维度前先纠正维度顺序
        if x.dim() == 4:
            B, D1, D2, D3 = x.shape  # 获取原始4维形状
            # print(f"进入嵌入层的原始维度: (B={B}, D1={D1}, D2={D2}, D3={D3})")

            # 目标：确保维度顺序为 (B, C, H, W)
            # 判断哪个维度是通道（输入层通常为3，中间层可能为64、128等）
            # 这里我们通过寻找合理的空间维度（较大的两个值）来判断
            dim_values = [D1, D2, D3]
            # 找到两个最大的维度作为空间维度（H和W）
            sorted_dims = sorted(enumerate(dim_values), key=lambda x: x[1], reverse=True)
            spatial_indices = [i for i, _ in sorted_dims[:2]]  # 空间维度索引
            channel_index = [i for i, _ in sorted_dims[2:]][0]  # 通道维度索引

            # 构建转置顺序：(B, 通道, 高度, 宽度)
            permute_order = [0, channel_index + 1]  # +1因为第0维是B
            for i in range(1, 4):
                if (i - 1) in spatial_indices and i not in permute_order:
                    permute_order.append(i)

            # 执行转置
            x = x.permute(permute_order)
            # print(f"嵌入层维度纠正: 转置顺序 {permute_order} → 新形状 {x.shape}")

        # 获取纠正后的维度信息 (批次大小, 通道数, 高度, 宽度)
        B, C, H, W = x.shape

        # 检查输入图像的空间尺寸是否与模型预期一致
        # print(f"输入图像尺寸: 高度={H}, 宽度={W}, 通道数={C}")
        # print(f"模型预期尺寸: {self.img_size[0]}x{self.img_size[1]}")
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"输入图像空间尺寸 ({H}x{W}) 与模型预期 ({self.img_size[0]}x{self.img_size[1]}) 不匹配。"

        # 投影操作：将图像分割为补丁并映射到特征空间
        x = self.proj(x).flatten(2).transpose(1, 2)  # 形状变为 (B, Ph*Pw, C)
