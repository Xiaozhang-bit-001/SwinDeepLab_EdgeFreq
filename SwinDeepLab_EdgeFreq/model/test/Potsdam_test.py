...
# ================= 导入你的模型 =================
PROJECT_ROOT = r'.../data/DeepLab_better/model2'
...
TEST_LIST = r'.../Dataset/Potsdam/lists_txt_RGB_improved/test.txt'
STATS_PATH = r".../Dataset/Potsdam/rgb_data_stats_improved.npz"
MODEL_WEIGHT_PATH = r".../ComparedModels/DeepLabReg_Pots_new_networks/DeepLabReg_Pots_256_256/iter30k_epo150_bs8_lr0.01_s1234/best_model.pth"
OUTPUT_DIR = r'.../TestResults/DeepLabReg_Potsdam'
...
# ======================== 2. Potsdam 数据加载 ========================
class PotsTestDataset(torch.utils.data.Dataset):
    def __init__(self, root_path, list_path, img_size=256, transform=None):
        ...
        print(f"📁 成功从列表加载 {len(self.file_names)} 个Potsdam测试样本")
...
class SegMetrics:
    ...
    def compute(self):
        ...
        results['mPrecision_list'] = precision
        results['mRecall_list'] = recall
        results['mF1_list'] = f1
...
def vis_result(img, mask, pred, file_name, save_path, mean, std):
    ...
    axes[2].set_title('DeepLabReg Prediction (Potsdam)', fontsize=12)
    axes[2].axis('off')
...
def main():
    ...
    print("\n🚀 开始Potsdam推理测试...")
    with torch.no_grad():
        for batch_idx, (imgs, masks, file_names) in enumerate(tqdm(test_loader, desc='Testing Potsdam DeepLabReg')):
            imgs = imgs.to(DEVICE)
    ...
    print("\n" + "=" * 60)
    print("📊 DeepLabReg Potsdam 模型测试结果汇总")
    print("=" * 60)
    print(f"总体精度 (OA)    : {results['OA']:.4f}")
    print(f"平均 IoU (mIoU)  : {results['mIoU']:.4f}")
    print(f"平均 Precision   : {results['mPrecision']:.4f}")
    print(f"平均 Recall      : {results['mRecall']:.4f}")
    print(f"平均 F1 (mF1)    : {results['mF1']:.4f}")
    
    print("\n📋 每类详细指标:")
    for cls_name in CLASS_NAMES:
        print(f"  {cls_name.ljust(18)} IoU: {results['per_class_iou'][cls_name]:.4f} | Prec: {results['per_class_precision'][cls_name]:.4f} | Rec: {results['per_class_recall'][cls_name]:.4f} | F1: {results['per_class_f1'][cls_name]:.4f}")

    metrics_path = os.path.join(OUTPUT_DIR, 'test_metrics.json')
    with open(metrics_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=4)

    print(f"\n✅ Potsdam测试完全结束！所有指标保存在: {metrics_path}")
