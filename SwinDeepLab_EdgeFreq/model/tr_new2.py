...
from dataset_synase2 import Synapse_dataset, RandomGenerator
...
def init_result_csv(save_path, num_classes):
    ...
    logging.info(f"✅ 初始化训练结果CSV文件：{save_path}")
...
def trainer_synapse(args, model, snapshot_path, start_epoch=0, train_mean=None, train_std=None):
    ...
    if start_epoch > 0:
        ...
        if ckpt_files:
            ...
            logging.info(f"✅ 已恢复优化器状态，从epoch {start_epoch} 开始训练")
    ...
    for epoch_num in tqdm(range(start_epoch, max_epoch), ncols=70):
        ...
        if mean_iou > best_mIoU:
            ...
            logging.info(f"\n🎉 【最优模型更新】Epoch {current_epoch} | mIoU = {best_mIoU:.4f}\n")

    # 训练结束输出最优结果
    logging.info("\n" + "="*60)
    logging.info(f"🎉 训练全部完成！总共 {max_epoch} 轮")
    logging.info(f"🏆 最优模型在 Epoch {best_epoch}")
    logging.info(f"🎯 最优 mIoU = {best_mIoU:.4f}")
    logging.info(f"📁 最优模型已保存：{os.path.join(snapshot_path, 'best_model.pth')}")
    logging.info("="*60)
